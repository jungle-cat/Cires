...
{
	std::map<std::string, std::vector<std::string> >& data = o_data;
    ...
    for (it = data.begin(); it != data.end(); ++it) {
        ...
        for (size_t j = 0; j < keysVector.size(); j++)
        	keysVector[j] = del_space(keysVector[j]);
        values_buffer = del_space(it->second[0]);

        if (!values_buffer.empty()) {
        	auto val = it->second;
        	val[0] = del_space(val[0]);
        	m_data[keys_buffer] = val;
        }
    }
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        for (it =  data.begin(); it!= data.end(); ++it) {
            ...
            if (curName == keysVector[0] || curName == keysVector[1]) {
            	auto val = it->second;
            	val[0] = del_space(val[0]);

            	if (hasValueThroughEq)
            		val[0] = del_space(buffer);
            	m_data[keys_buffer] = val;
            	break;
            }
...
bool CommandLineParser::has(const std::string& keys)
{
    ...
    std::string key = del_space(keys);

    for(it = m_data.begin(); it != m_data.end(); it++)
    {
        keysVector = split_string(it->first, "|");
        for (size_t i = 0; i < keysVector.size(); i++) {
        	keysVector[i] = del_space(keysVector[i]);
//        	std::cerr << "\t: " << keysVector[i] << std::endl;
        }

        std::string val = del_space(it->second[0]);

        if (keysVector.size() == 1) keysVector.push_back("");

        if (((key.compare(keysVector[0]) == 0) ||
            (key.compare(keysVector[1]) == 0)) )
            return true;
    }
 ...
 _Tp CommandLineParser::fromStringNumber(const std::string& str)//the default conversion function for numbers
{
 ...
 void CommandLineParser::printParams()
 {
    ...
    std::vector<string> keysVector;
    std::string buf;
    for(it = o_data.begin(); it != o_data.end(); it++)
    {
        keysVector = split_string(it->first, "|");
